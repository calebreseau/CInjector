...
//result:=QueueUserAPC(lploadLibraryAddr,hthread,nativeuint(lpDllAddr));
end;
...
begin
  result:=true;
  //if SuspendThread(hthread )<>0 then outputdebugstring(pchar('suspend failed: '+inttostr(getlasterror)));
...
begin
	...
	loadlibrarypointer:=getprocaddress(getmodulehandle('kernel32.dll'),'LoadLibraryA');
        ...
        //status:=ntallocatevirtualmemory(ahandle,alloc,0,@size,MEM_COMMIT or MEM_RESERVE, PAGE_EXECUTE_READWRITE);
        if alloc=nil
        ...
        //readprocessmemory(ahandle,alloc,@buf[1],size,byteswritten);
	...
	status:=rtlcreateuserthread(ahandle,nil,susp,0,0,0,loadlibrarypointer,alloc,@th,@ch);
        //getthreadinformation(ahandle,$2,@gtiresult,sizeof(dword));
        //resumethread(th);
